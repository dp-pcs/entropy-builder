...
def test_status_api_returns_job_state(client, mocker):
    ...
    resp = client.get(f"/api/job/{_VALID_JOB_ID}/status")
...
def test_status_api_includes_download_url_when_complete(client, mocker):
    mocker.patch("webapp.main.s3.read_job_state", return_value={
        "job_id": _VALID_JOB_ID, "status": "complete", "step": "complete",
        "step_index": 7, "total_steps": 7, "gaps": [], "gap_s3_keys": [],
        "vault_key": f"jobs/{_VALID_JOB_ID}/entropy-vault.zip",
        "claude_settings_key": f"jobs/{_VALID_JOB_ID}/claude_settings.json",
...
def test_status_api_rejects_invalid_job_id(client):
...
def test_status_page_route_exists(client, mocker):
    ...
    assert resp.status_code == 200
    assert b"Building your system" in resp.content


def test_gap_presign_stores_key(client, mocker):
    mocker.patch("webapp.main.s3.read_job_state", return_value={
        "job_id": _VALID_JOB_ID, "status": "processing", "gap_s3_keys": [],
    })
    ...
    resp = client.post(f"/api/job/{_VALID_JOB_ID}/gaps/presign", json={
    ...
    written_state = webapp.main.s3.write_job_state.call_args[0][1]
    assert any("additional-notes.pdf" in k for k in written_state["gap_s3_keys"])
